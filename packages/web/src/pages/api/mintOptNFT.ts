...
const API_URL = process.env.NEXT_PUBLIC_OPT_URL || "hogehoge";
const provider = new providers.JsonRpcProvider(API_URL);
const contract = require('../../contracts/VminaNFT.json');
const contractAddress = process.env.NEXT_PUBLIC_OPT_CONTRACT_ADDRESS || "0x";
...
const mint = async (address: string, tokenUri: any) => {
  const receipt = await VNFTContract.mintNFT(address, tokenUri, { gasLimit: 1000000 })
  return receipt
}
