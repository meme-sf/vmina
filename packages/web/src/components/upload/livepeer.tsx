...
import { Badge, Box, Button, Center, Text, Modal, ModalOverlay, ModalContent, ModalBody, Icon } from '@chakra-ui/react';
import {
  createReactClient,
  studioProvider,
  useAsset,
...
const CreateAndViewAsset: React.FC<Props> = ({ setStep, images, setImages, video, setVideo, b64, setB64, canvasses, setCanvasses }) => {
    ...
    <>
      <Box ref={finalRef}>
        <Modal finalFocusRef={finalRef} isOpen={loading} onClose={() => {setLoading(false)}}>
          ...
          <ModalContent bg='white' w='90%' h='500px' borderRadius='20px' top='100px'>
            <ModalBody paddingInline='0'>
              <Box mt='20px'>
                <Text color='black' textAlign='center' fontWeight='bold' fontSize='25px'>
                  Uploading the video to Livepeer...<br />
                ...
                <Center>
                  ...
                  <ProgressBar
                    ...
                    baseBgColor='#FBD28D'
                  />
                  <Icon ml='5px' w='20px' h='20px' color='#039D06' opacity={uploadDone ? '1' : '0'} as={AiOutlineCheckCircle} />
                </Center>
                <Center>
                <Text color='black'>Process:&nbsp;&nbsp;</Text>
                  <ProgressBar
                    ...
                    baseBgColor='#FBD28D'
                  />
                <Icon ml='5px' w='20px' h='20px' color='#039D06' opacity={processDone ? '1' : '0'} as={AiOutlineCheckCircle} />
                </Center>
                <Center>
                  <Button
                    mt='50px'
        ...
        <Center>
          <Box>
            <Center>
              {video ? (
                <>
                  <Badge>{video.name}</Badge><br />
                </>
              ) : (
                <>
                  <Text>Select a video to upload.</Text><br />
                </>
              )}
            </Center>
