          ...
          {video.imagePaths.map((val: string, key: number) => {
            return (
              <Center key={key} w='100%'>
                <Image src={`https://ipfs.io/ipfs${val}`} alt='image test' key={key} w='100%' height='200px' m='0 auto' />
              </Center>
            )
          })}
        </Slider>
        <Center color='black' justifyContent={'space-between'} p='0 20px' mt='30px' fontSize='18px' fontWeight='bold'>
          <Center gap={1}>
